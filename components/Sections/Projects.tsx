    ...
    <div className={`relative mb-32 flex ${isEven ? 'justify-start' : 'justify-end'} w-full`}>
      <div className="w-full md:w-2/3 lg:w-1/2 bg-lab-surface/90 backdrop-blur border border-lab-dim/20 p-8 rounded-sm hover:border-lab-dim/50 transition-colors duration-300">
        <div className="flex items-center justify-between mb-4">
          <span className={`font-mono text-xs px-2 py-1 bg-white/5 rounded ${project.color}`}>
            {project.category}
          </span>
        ...
        <h4 className={`text-lg font-mono mb-6 ${project.color} opacity-80`}>{project.subtitle}</h4>

        <div className="mb-6 space-y-2 text-gray-400 font-light text-sm">
          {project.description.map((desc, i) => (
            <p key={i} className="flex gap-2">
              <span className="text-lab-dim">›</span> {desc}
            </p>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            <h5 className="text-xs font-mono text-lab-dim mb-2 uppercase tracking-wider">Focus</h5>
            <ul className="text-sm text-gray-300 space-y-1">
              {project.focus.map(f => <li key={f}>• {f}</li>)}
            </ul>
          </div>
          <div>
             <h5 className="text-xs font-mono text-lab-dim mb-2 uppercase tracking-wider">Tech</h5>
             <div className="flex flex-wrap gap-2">
               {project.tech.map(t => (
                 <span key={t} className="text-xs border border-lab-dim/40 px-2 py-0.5 rounded text-gray-400">{t}</span>
               ))}
             </div>
          </div>
        </div>
    ...
    <SectionWrapper id={SectionId.PROJECTS} className="z-10 pointer-events-none">
      <div className="pointer-events-auto w-full">
        <div className="mb-20 text-center">
          ...
          <p className="text-gray-400 font-mono text-xs">DEPLOYED SYSTEMS & EXPERIMENTS</p>
        </div>
        ...
        <div className="max-w-3xl mx-auto border-t border-lab-dim/20 pt-16">
           <h3 className="text-xl font-bold text-center mb-8 flex items-center justify-center gap-2">
             <span className="w-2 h-2 bg-lab-dim rounded-full"></span>
             Prototype Archives
           </h3>
